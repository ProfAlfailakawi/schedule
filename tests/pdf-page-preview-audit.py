...
ok('11 multi-page page 1 keeps the exact one-page course-key path while later pages retain rescue', 'const courseKeysForPage=(index:number)=>images.length>1&&index===0?[]:multiPageCourseKeys' in ocr and 'readGrid(upright,lanePool,authorityGridDepartment,pageCourseKeys)' in ocr and 'courseKeysForPage(index)' in ocr)
