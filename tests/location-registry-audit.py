...
ok('96p an identity unique in the whole system register proves itself: the publish gate accepts it without department history, scope-dependent matches still need the department', 'instructorProvenByFullName=["EXACT_FULL","FACULTY_IDENTITY","GLOBAL_THREE_NAME","GLOBAL_SOLE_TWO_NAME"]' in server.replace(' ','') and '!instructorChosenByHand&&!instructorProvenByFullName&&!departmentInstructorIds.has' in server)
...
ok('96t every instructor reads their department on the card and in the details pane, teaching and secondment apart', 'fetch("/api/instructor-affiliations")' in (ROOT/'src/components/Instructors.tsx').read_text() and 'const homeLabel = (id: number)' in (ROOT/'src/components/Instructors.tsx').read_text() and '<span>يدرّس في</span>' in (ROOT/'src/components/Instructors.tsx').read_text() and '<span>منتدب لدى</span>' in (ROOT/'src/components/Instructors.tsx').read_text())
