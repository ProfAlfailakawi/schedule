...
# Publishing replaces whole departments and the change report scans stored versions: heavy work behind a trusted account is still bounded.
ok('130e whole-department publishing and the change report carry a generous per-account rate limit', 'function rateLimitHeavy' in server and 'drafts/:id/publish", requirePermission(7), rateLimitHeavy' in server and 'authority-pdf-diff", requireAnyPermission([7,8,9,10,14,16,17]), rateLimitHeavy' in server and 'HEAVY_RATE_LIMIT_MAX' in server)
