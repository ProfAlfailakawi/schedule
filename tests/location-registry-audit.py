...
ok('137 multi-page scan partitions the proven OCR pool into two isolated page lanes without spawning a second engine', 'GOLDEN PAGE LANES' in doc_ocr and 'getFastLaneWorkerPool' not in doc_ocr and 'const laneA:OcrWorkerPool={eng:pool.eng.slice' in doc_ocr and 'const laneB:OcrWorkerPool={eng:pool.eng.slice' in doc_ocr and 'await Promise.all([laneEven(),laneOdd()])' in doc_ocr)
...
ok('139 graduate request keeps approved reason plus a bounded requirement tied to it', 'اكتب متطلباتك بعد اختيار نوع الطلب' in server and 'details:graduateDetails' in server and 'wireGraduateDetails' in server and 'student-graduate-request' in intel)
...
ok('142 manual rows are permanently post-PDF green and sourceOrder cannot impersonate PDF provenance', 'sourceOrder: Math.max(1_000_000, Date.now())' in server and 'Number(rawOrder)<1_000_000&&hasPdfEvidence(row)' in server and 'isImportedPdfRow(row)&&Number(row?.sourceOrder)===order' in server)
ok('143 reference number remains readable on deleted rows and has a live-row fallback', '.authority-pdf-row-deleted>[role="cell"].authority-pdf-reference-number{text-decoration:none!important}' in print_css and '(row as any).referenceNumber' in authority_report)
