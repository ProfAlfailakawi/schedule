...
ok('66 authority PDF never trims a 7-character building bleed into canonical data', 'No “delete the last digit” repair exists here' in server and 'const bleed=token.match' not in server and '012B091 is NOT repaired into 012B09' in doc_ocr)
...
# Regression guards for the restored professional import path.
schedules_src=(ROOT/'src/components/Schedules.tsx').read_text()
quick_src=(ROOT/'src/components/QuickCreatePopover.tsx').read_text()
ok('70 every new course restarts sections at 501 and uses the first free 500-series number in UI and backend', 'let candidate = 501' in schedules_src and 'while (used.has(candidate)) candidate += 1' in schedules_src and 'value >= 501' in schedules_src and '501, 502, 503' in quick_src and 'let nextCourseSection=501' in server and 'while(usedCourseSections.has(nextCourseSection))nextCourseSection+=1' in server and 'SCode: canonicalNewSectionCode' in server)
ok('71 imported authority section numbers are preserved exactly instead of being regenerated', 'normalizeImportSectionSeries' in intel and 'SCode:String(row.SCode||"").trim()' in intel and 'SCode:grid.scode' in doc_ocr)
ok('72 building and room stay in their physical cells next to time', 'const geometricBuilding=timeIndex>=0&&timeIndex+1<columnBands.length?timeIndex+1:-1' in doc_ocr and 'const geometricHall=timeIndex>=0&&timeIndex+2<columnBands.length?timeIndex+2:-1' in doc_ocr and 'Never rescue one field from another column' in doc_ocr)
