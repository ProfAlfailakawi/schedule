...
ok('96p a full-name identity proves itself: the publish gate accepts it without department history, weaker matches still need the department', 'instructorProvenByFullName=["EXACT_FULL","FACULTY_IDENTITY"]' in server.replace(' ','') and '!instructorChosenByHand&&!instructorProvenByFullName&&!departmentInstructorIds.has' in server)
ok('96q the preview stops flagging a full-name identity and states its provenance instead', 'instructorOutsideRegister(row) && !provenByFullName(row)' in (ROOT/'src/components/ImportPreviewTable.tsx').read_text() and 'import-registry-badge' in (ROOT/'src/components/ImportPreviewTable.tsx').read_text() and '.import-registry-badge{' in (ROOT/'src/styles/06-intelligence.css').read_text())
