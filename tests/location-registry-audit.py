...
ok('71 header college mismatch blocks before full OCR', 'PDF_COLLEGE_MISMATCH' in server and server.index('PDF_COLLEGE_MISMATCH') < server.index('ocrDocument(bytes,"application/pdf"'))
ok('72 partial embedded header cannot skip scanned header completion', 'Partial evidence is useful, but it is NOT enough to skip the scan header' in doc_ocr and 'embedded.term&&embedded.college&&embedded.branch&&embedded.department' in doc_ocr)
ok('73 branch code alone cannot waive site identity', r'(\d{3,4})' in doc_ocr and 'PDF_HEADER_SITE_UNRESOLVED' in server and 'if(!sourceSite)' in server)
ok('74 scan table failure is marked suspicious and authority import blocks it', 'gridDetected:false' in doc_ocr and 'suspicious:true' in doc_ocr and 'SUSPICIOUS_EXTRACTION' in server)
ok('75 scan room and building are anchored to physical cells next to time', 'const geometricBuilding=timeIndex>=0&&timeIndex+1<columnBands.length?timeIndex+1:-1' in doc_ocr and 'const geometricHall=timeIndex>=0&&timeIndex+2<columnBands.length?timeIndex+2:-1' in doc_ocr)
ok('76 section is source-preserving 5xx and never auto-renumbered', 'Preserve the printed 5xx value exactly' in doc_ocr and 'SCode:/^5\\d{2}$/.test(candidate)?candidate:""' in intel and 'normalizeAuthoritySectionSeries' not in doc_ocr and 'normalizeAuthoritySectionSeries' not in server)
ok('77 unknown course stays unresolved instead of neighbour/fuzzy assignment', 'Neighbouring rows and edit-distance' in doc_ocr and 'AdCourseId:0' in doc_ocr)
ok('78 instructor authority matching is department-scoped even for exact global names', 'Even an exact university-wide name' in doc_ocr and 'if(preferredIds)' in doc_ocr and 'requireDepartmentInstructor:true' in server)
ok('79 هيئة تدريسية maps only to its exact canonical identity and department evidence', 'rawClean==="هيئه تدريسيه"' in doc_ocr and 'clean(person.AdInstructorName)==="هيئه تدريسيه"' in doc_ocr and 'if(preferredIds)return undefined' in doc_ocr)
ok('80 instructor ambiguity requires a stronger winner margin', 'top.score>=0.70' in doc_ocr and 'top.score-runner.score>=0.15' in doc_ocr)
ok('81 count reconciliation blocks silent row loss', 'COUNT_RECONCILIATION_FAILED' in server and 'parsed.rows.length<Math.ceil(geometryRows*.7)' in server)
ok('82 authority import receipt binds reviewed header to exact context', 'PdfImportReceipt' in server and 'verifyPdfImportReceipt' in server and 'PDF_IMPORT_RECEIPT_REQUIRED' in server and 'PDF_IMPORT_RECEIPT_INVALID' in server)
ok('83 preview and backend both require department instructor evidence', '!departmentIds.includes(Number(row.AdInstructorId))' in imp and 'الأستاذ المطابق غير مثبت ضمن القسم الحالي' in server)
ok('84 cross-branch badge is rendered beside the course row', 'import-course-scope-note' in imp and 'scopeMismatchType === "CROSS_BRANCH"' in imp)
ok('85 required import cells share explicit red-state checks', all(x in imp for x in ['missing.course(row)','missing.scode(row)','missing.days(row)','missing.time(row)','missing.building(row)','missing.room(row)','missing.instructor(row)']))
ok('86 canonical room resolution always depends on confirmed building', 'resolveRoom(registry,rawHall,building.value.id' in server and 'row.buildingId=building.value.id' in server)
...
ok('90 time anchor requires an actual separator and cannot treat six merged digits as time', 'time:/^(?:[0-2]\\d[0-5]\\d\\s*[-–—~]' in doc_ocr and 'optional-separator version could misclassify' in doc_ocr)
ok('91 arbitrary six-digit OCR cannot become a building', '(?:0510|0520|0410|0420)\\d{2}' in doc_ocr and 'Arbitrary six-digit OCR such as 345045 is not location evidence' in doc_ocr)
ok('92 authority scan skips unsafe whole-page text-flow fallback', 'else if(options.authorityTable)' in doc_ocr and 'DO NOT spend' in doc_ocr and '{authorityTable:true}' in server)
ok('93 authority section backend rejects units-hours drift such as 3', 'if (!/^5\\d{2}$/.test(String(row.SCode || "")))' in server and 'لم يتم توليد رقم بديل' in server)
ok('94 unresolved OCR building is retained only as evidence not visible canonical data', 'row.AdRoomCode="";row.AdRoomHall=""' in server and 'Keep OCR garbage only as evidence' in server)
ok('95 scanned header uses Arabic-only OCR and a wider header-only crop', 'createWorker("ara")' in doc_ocr and 'image.height*0.32' in doc_ocr and 'collegeFromBranch' in doc_ocr)
ok('96 UI explains header-first preflight before full table read', 'يتحقق من ترويسة الملف قبل قراءة الجدول' in intel)
types_src=(ROOT/'src/types.ts').read_text()
ok('97 ScheduleDraft persists the signed PDF import receipt contract', 'importReceipt?: string;' in types_src and 'Server-signed proof that the PDF header matched this exact academic scope.' in types_src)
