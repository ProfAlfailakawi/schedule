...
ok('126 authority change report preserves source PDF column order including section', '["رقم المقرر", "الرقم المرجعي", "الشعبة", "مسمى المقرر", "عدد الوحدات", "عدد الساعات", "الحد الأقصى", "القاعة", "المبنى", ...(showSite ? ["الموقع"] : []), "الوقت", "الأيام", "المدرس"]' in authority_report)
...
ok('130 Authority diff engine can still represent full deletion against an empty live timetable', 'if(!live.length){res.status(404)' not in server and 'buildAuthorityPdfDiff(scopedBaseline,live,{instructorNameById})' in server and 'authorityBaselineForScope(draft.baselineRows||[],draft,collegeId,sectionId)' in server)
...
ok('130x the change report across sites is ONE merged document with a site column, not three reports glued together', 'rows: authorityBook.flatMap(entry => entry.report.rows.map(row => ({ ...row, siteLabel: entry.site.siteLabel })))' in reports_branch and 'showSite = false,' in authority_branch and '.authority-pdf-with-site .authority-pdf-grid' in print_css)
...
ok('130w the branch document is ONE table in the imported document order, with a site column that exists only when sites do', 'const importOrder = (row: FSchedule)' in reports_branch_ui and 'const showSite = bookSites.length > 1;' in reports_branch_ui and '...(showSite ? ["الموقع"] : [])' in reports_branch_ui and '.print-comprehensive-with-site .print-comprehensive-grid-row{' in (ROOT/'src/styles/08-print.css').read_text())
