...
ok('96l import selection stays inside the department and has no outside-identity confirmation', '<InstructorPicker departmentOnly canCreate={false}' in imp and 'className="import-instructor-confirm"' not in imp and 'if (departmentOnly || needle.length < 2)' in (ROOT/'src/components/InstructorPicker.tsx').read_text())
...
ok('96p the publish gate requires department membership even for manual or exact-name matches', 'options.requireDepartmentInstructor&&!departmentInstructorIds.has(Number(row.AdInstructorId))' in server and 'instructorChosenByHand' not in server and 'instructorProvenByFullName' not in server)
ok('96q matching uses only the department catalogue without a college fallback or registry badge', 'const instructors = allInstructors.filter(person => preferredInstructorIds.has(Number(person.AdInstructorId)))' in server and 'collegeFallbackInstructorIds' not in server and 'import-registry-badge' not in imp)
