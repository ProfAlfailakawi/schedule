...
ok('84 instructor import uses unique full/two/three-name system identity with no edit-distance rescue', 'TWO or THREE real name tokens' in doc and 'ambiguous names stay blank' in doc and 'return choose(catalogue,false)' in doc)
...
ok('96 distinctive confirmed room may rescue a damaged building only when the room fingerprint is unique', 'resolveBuildingFromUniqueRoom' in location_registry and 'buildingFromRoom' in server and 'Ambiguous rooms' in location_registry)
ok('97 instructor identity canonicalizes spaced/unspaced عبد names and keeps two-name proof department-only', 'Authority/system spellings alternate constantly between «عبد الله» and' in doc_ocr and 'const twoExactProof=allowTwo' in doc_ocr and 'return choose(catalogue,false)' in doc_ocr)
