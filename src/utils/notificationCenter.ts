...
export function buildNotifications(input: CenterInput): CenterNotification[] {
  ...
  if (REGISTRAR.has(role) || WATCHERS.has(role)) {
    ...
    /* ── موعدٌ لم يُحدَّد ──────────────────────────────────────────────────
       الموعدُ الواحد لكل الأقسام بيد رئيس التسجيل، وبدونه لا «متأخّر» ولا
       تنبيهٌ لعميد. فما دام فصلٌ فيه أقسامٌ لم تسلّم بلا موعد، يُطلب منه. */
    if (role === "registrarHead" && notYet > 0 && !input.deadline?.effective) {
      items.push({
