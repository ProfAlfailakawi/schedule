...
export function reachAboutCard(
  ...
  const text = [
    ...
    detail || "",
    cardUrl,
    occasion === "decided" ? "افتح الرابط لترى القرار في كل بند، والبدائل إن وُجدت." : "الرابط شخصيٌّ لك، للقراءة فقط ويُحدَّث من نفسه حتى نهاية الفصل — احفظه ولا حاجة لطلبه مرة أخرى.",
  ].filter(Boolean).join("\n");
