...
export const GUIDE_FEATURES: GuideFeature[] = [
  ...
  /* ── دورة الاعتماد ────────────────────────────────────────────────────────
     شاشةٌ واحدة بوجهين: التسجيل يراجع الوارد، والقسم يردّ على ملاحظاته. ولذلك
     صلاحيتها ليست رقم شاشةٍ بل صفةُ صاحب الحساب — والمرشد لا يعرض ما لا يفعله
     صاحبه، فتُترك بلا `permission` ويحكمها ظهورُ الأيقونة نفسها. */
  { id:"page.scheduleChanges", title:"تغييرات الجدول", summary:"ما تحرّك في جدول القسم منذ مراجعة التسجيل الأخيرة، وملاحظاتُه وقرارُه.", view:"scheduleChanges", group:"العمل اليومي", keywords:["تغييرات","وارد","تسجيل","ملاحظات","اعتماد","إرجاع","جولة"], version:1 },
  { id:"changes.action.note", title:"ملاحظة على خانة", summary:"ينقر موظّف التسجيل على الخانة نفسها — القاعة أو الوقت أو الأستاذ — فتصل الملاحظة إلى موضعها من جدول القسم. النصّ اختياري.", view:"scheduleChanges", group:"دورة الاعتماد", keywords:["ملاحظة","تعليق","خانة","قاعة","وقت"], version:1, target:"changes.action.note", risk:"write", steps:[{target:"changes.action.note",text:"اضغط اسم الخانة التي عليها الملاحظة؛ الخانة نفسها هي الرسالة، والنصّ زيادةٌ عند الحاجة."}] },
  { id:"changes.action.return", title:"إرجاع الجدول للقسم", summary:"يُرجع الجدول بملاحظاته ويفتح القفل عن القسم. ولا يُرجَع جدولٌ بلا ملاحظةٍ واحدة على الأقل.", view:"scheduleChanges", group:"دورة الاعتماد", keywords:["إرجاع","رد","ملاحظات","جولة"], version:1, target:"changes.action.return", risk:"write", undoable:false, steps:[{target:"changes.action.return",text:"اكتب ملاحظةً واحدةً على الأقل، ثم أرجِع الجدول. تُرسل الملاحظات كلها دفعةً واحدة."}] },
  { id:"changes.action.accept", title:"قبول الجدول نهائياً", summary:"اعتماد جدول القسم. لا يُقبل جدولٌ فيه تعارضٌ مادّي؛ أمّا الملاحظات اللائحية فتُعرض ولا تمنع.", view:"scheduleChanges", group:"دورة الاعتماد", keywords:["قبول","اعتماد","نهائي"], version:1, target:"changes.action.accept", risk:"write", steps:[{target:"changes.action.accept",text:"راجع ما تغيّر، ثم اعتمد. وأيُّ تعديلٍ بعد القبول يعيد الجدول إليك جولةً جديدة."}] },
  { id:"changes.action.deadline", title:"موعد تسليم الجداول", summary:"لوحة «مواعيد التسليم» أعلى تغييرات الجدول: يضع رئيس التسجيل آخر موعدٍ للفصل مرّةً، فيظهر صريحاً لكل قسم فوق جدوله.", view:"scheduleChanges", group:"دورة الاعتماد", keywords:["موعد","تسليم","تاريخ","آخر موعد","مواعيد التسليم"], version:2, target:"changes.action.deadline", risk:"write", steps:[{target:"changes.action.deadline",text:"اختر «بعد أسبوع» أو «بعد أسبوعين» أو تاريخاً آخر واحفظ. ينتهي الموعد 23:59 بتوقيت الكويت، وبعده لا يُستورد ملفٌّ ولا يُنسخ فصل."}] },
  { id:"changes.action.exceptions", title:"استثناءات موعد التسليم", summary:"أيامٌ إضافية لكل الأقسام أو لكلية أو لأقسامٍ بعينها، بسببٍ يراه القسم، ومنحُ طلبات التمديد أو رفضُها بضغطة.", view:"scheduleChanges", group:"دورة الاعتماد", keywords:["تمديد","استثناء","أيام","طلب تمديد","مهلة"], version:1, target:"changes.action.exceptions", risk:"write", steps:[{target:"changes.action.exceptions",text:"اضغط «إضافة استثناء»، اختر الأقسام، ثم «+3 أيام» أو تاريخاً، واكتب السبب. تُعدّ الأيام من موعد الفصل، أو من استثناء القسم القائم إن كان أبعد."}] },
  /* ── رغبات الأساتذة ──────────────────────────────────────────────────────
     دورةٌ تسبق دورةَ الاعتماد: الأساتذة يقولون رأيهم في مسوّداتهم، ثم يُرفع
     الجدولُ إلى التسجيل. وصلاحيتُها صلاحيةُ الورشة نفسها (٧): من يبني الجدول
     هو من يثبّت ويرفض، لأن القاعات قرارُه. */
  { id:"page.instructorRequests", title:"رغبات الأساتذة", summary:"ما طلبه الأساتذة على مسوّدات جداولهم: ما ثُبّت، وما رُفض ولماذا، ومن لم يفتح رابطه بعد.", view:"instructorRequests", group:"العمل اليومي", keywords:["رغبات","أساتذة","طلبات","مسودة","وارد","تعديل"], version:1, permission:7 },
  { id:"requests.action.fix", title:"تثبيت طلب أستاذ", summary:"يحفظ التعديل في الجدول عبر مسار الحفظ نفسه، فيظهر في تقرير التغييرات تلقائياً، ثم يُسجَّل القرار في طلب الأستاذ.", view:"instructorRequests", group:"رغبات الأساتذة", keywords:["تثبيت","قبول","حفظ","تعديل"], version:1, permission:7, target:"requests.action.fix", risk:"write", steps:[{target:"requests.action.fix",text:"البندُ المتعارض لا يُثبَّت: عالجه أو ارفضه بسبب. وما يُثبَّت يُكتب في الجدول أولاً، ولا يُسجَّل «ثُبّت» إلا إذا تحرّك الجدول فعلاً."}] },
  { id:"requests.action.reject", title:"رفض طلب أستاذ", summary:"يُسجَّل الرفض بسببٍ من قائمةٍ مغلقة، ومعه بدائلُ يقترحها النظام. ولا يُكتب في الجدول شيء: لم يتغيّر شيء.", view:"instructorRequests", group:"رغبات الأساتذة", keywords:["رفض","سبب","بديل","اعتذار"], version:1, permission:7, target:"requests.action.reject", risk:"write", steps:[{target:"requests.action.reject",text:"السببُ إلزاميّ: رفضٌ بلا سببٍ يدفع الأستاذ إلى الهاتف فتضيع الحجّة خارج النظام. والبدائلُ معروضةٌ سلفاً، توافق عليها أو تحذفها."}] },
  /* ── كشفُ التسجيل ────────────────────────────────────────────────────────
     آخِرُ حلقةٍ في السلسلة، والوحيدةُ التي يكتب فيها الطرفان: التسجيلُ صاحبُ
     القرار، ومن يبني الجدول يسلّم ويتابع. ولا إذنَ واحداً يجمعهما، فتُترك بلا
     `permission` ويحكمها ظهورُ الأيقونة نفسها كما في «تغييرات الجدول». */
  { id:"page.studentRegistration", title:"كشف التسجيل", summary:"ما طلبه الطلبة في الاستبيان: تنظر فيه لجنة القسم أولاً فتوافق أو لا توافق بسبب، وما توافق عليه وحده يصل التسجيل فيُسجَّل أو يُردّ.", view:"studentRegistration", group:"العمل اليومي", keywords:["تسجيل","طلبة","كشف","مقررات","حالة","رقم الحالة"], version:1 },
  { id:"registration.action.state", title:"حالة مقرّر عند التسجيل", summary:"يكتبها التسجيل على الكشف نفسه الذي يقرؤه القسم، فيراها القسمُ لحظتَها ويراها الطالبُ في صفحته. ولا تُكتب في جدولٍ ولا تحجز مقعداً.", view:"studentRegistration", group:"التسجيل", keywords:["سُجّل","ردّ","سبب","بانتظار","مقعد"], version:1, target:"registration.action.state", risk:"write", steps:[{target:"registration.action.state",text:"الردُّ يلزمه سبب: ردٌّ بلا سببٍ يُعيد الطالب إلى المكتب ليسأل «ليش؟»، وهو ما بُني الكشفُ ليُغنيَ عنه. وما تكتبه هنا قولٌ عن طلب الطالب، لا حجزُ مقعد."}] },
  { id:"approval.action.sign", title:"توقيع الجدول", summary:"يوقّع رئيس لجنة الجدول أولاً ثم رئيس القسم. التوقيع مرّةً واحدة عند أول إرسال، ومشدودٌ إلى نسخةٍ بعينها.", view:"schedules", group:"دورة الاعتماد", keywords:["توقيع","اعتماد","لجنة","رئيس القسم"], version:1, permission:7, target:"approval.action.sign", risk:"write", steps:[{target:"approval.action.sign",text:"التعارض المادّي وحده يمنع التوقيع؛ واللائحة تُعرض وتُسجَّل مع توقيعك ولا تمنعه."}] },
  { id:"approval.action.submit", title:"إرسال الجدول إلى التسجيل", summary:"يُرسل الجدول بعد اكتمال التوقيعين، ويقفل التعديل حتى يُقبل أو يُرجَع.", view:"schedules", group:"دورة الاعتماد", keywords:["إرسال","تسليم","تسجيل"], version:1, permission:7, target:"approval.action.submit", risk:"write", steps:[{target:"approval.action.submit",text:"بعد الإرسال يُقفل التعديل: الطرف الآخر يقرأ الجدول الآن، وتغييرُه تحت يده يجعل ملاحظته على شيءٍ لم يعد موجوداً."}] },
  ...
  { id:"schedule.publish", title:"نشر الجدول", summary:"بابان: جدولُ القسم لمن يحمل الرابط، وبطاقةُ الأستاذ يفتحها برقمه المدني — ومنها يطلب تعديل جدوله حين تُفتح طلبات التعديل. ولا ينفذ المرشد النشر تلقائيًا.", view:"schedules", group:"القرار", keywords:["نشر","رابط قراءة","مشاركة","اعتماد","رغبات","تعديل الأستاذ","بطاقة"], version:2, permission:7, target:"schedule.publish", risk:"sensitive", steps:[{target:"schedule.tool.more",text:"افتح «المزيد» لإظهار أدوات النشر."},{target:"schedule.publish",text:"هذا هو مدخل النشر. راجع النطاق ثم أكّد القرار بنفسك."}] },
  /* ── البابُ الذي يعدّل منه الأستاذ ────────────────────────────────────────
     البابان الآخران قراءةٌ فقط، فمن بحث عن موضع التعديل وجد بطاقةَ اطّلاعٍ
     وظنّها هي — وهو سؤالٌ وقع فعلاً. فيُسمّى الفرقُ هنا صريحاً، ويُقال أين
     تُتابَع الروابط بعد إصدارها. */
  { id:"schedule.publish.requests", title:"رغبات الأساتذة — إرسال المسوّدات", summary:"بطاقةُ الأستاذ هي باب الرغبات: رابطٌ واحدٌ للقسم، يفتح منه كل أستاذٍ جدولَه برقمه المدني ويطلب تعديله؛ لا يرى جدول غيره، ولا يختار قاعةً، والقرارُ يبقى للقسم.", view:"schedules", group:"القرار", keywords:["رغبات","الأساتذة","تعديل","طلب","مسودة","وين يعدل الدكتور","إرسال"], version:1, permission:7, target:"schedule.publish", risk:"sensitive", steps:[{target:"schedule.tool.more",text:"افتح «المزيد» لإظهار أدوات النشر."},{target:"schedule.publish",text:"افتح النشر: فيه بابان، جدولُ القسم وبطاقةُ الأستاذ."},{target:"schedule.publish.requests",text:"هذه هي «بطاقة الأستاذ» — ومنها يطلب الأستاذ تعديل جدوله. وجدولُ القسم بجانبها للاطّلاع وحده."},{target:"schedule.publish.requests",text:"أبقِ «استقبال طلبات تعديل الجدول» مفعّلاً واكتب آخر موعد، ثم أنشئ الرابط وأرسله. وتتابع من فتح ومن أرسل في وارد الأساتذة."}] },
  { id:"page.scheduleCopy", title:"نسخ فصل", summary:"نسخ جدول فصل إلى فصل آخر ضمن صلاحيات الإدارة.", view:"scheduleCopy", group:"الإدارة", keywords:["نسخ","فصل"], permission:7, rootOnly:true, version:2 },
  /* The same screen, reached from the system-administration rail. Both names
     are registered so the guide answers to whichever door the reader used. */
  { id:"page.copyTerm", title:"نسخ فصل", summary:"نسخ جدول فصل إلى فصل آخر — من داخل إدارة النظام.", view:"copyTerm", group:"الإدارة", keywords:["نسخ","فصل","إدارة النظام"], permission:7, rootOnly:true, version:1 },
