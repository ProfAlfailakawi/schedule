    ...
    <>
      ...
      <aside hidden={drawerHidden} className={`smart-guide visual-minimal no-print level-${sheetLevel} ${drawerHidden ? "is-screen-action" : ""}`} ref={drawerRef} role="dialog" aria-label="مرشد SCHEDULE" aria-modal={!drawerHidden && !pointMode && !located && !preview} aria-hidden={drawerHidden || pointMode || located || preview ? true : undefined} tabIndex={-1} dir="rtl">
        ...
        <nav className="smart-guide-browse" aria-label="أقسام المرشد">
          ...
          <button className={browseMode === "new" ? "active" : ""} type="button" title="ما استُجدّ ولم تره بعد" onClick={() => selectBrowseMode("new")}><Sparkles /><span>الجديد</span>{unreadSummary.total ? <i title={`${countOf(unreadSummary.total, AR.element)} ${nounFor(unreadSummary.total, AR.newAdj)}`}>{formatGuideCount(unreadSummary.total)}</i> : null}</button>
          ...
          <>
            {browseMode === "forYou" ? (
              <>
                ...
                {routines.length ? <details className="smart-guide-fold"><summary><Zap /><span><small>اختصاراتك</small><strong>مساراتك المحفوظة</strong></span><b dir="ltr">{routines.length}</b></summary><div className="smart-guide-routines">{routines.slice(0,5).map(routine => <article key={routine.id}><span><Zap /></span><div><strong>{routine.name}</strong><small>{countOf(routine.sequence.length, AR.step)} · آمن</small></div><button type="button" onClick={() => runRoutine(routine)}><Play />تشغيل</button><button type="button" className="danger" onClick={() => { removeGuideRoutine(userId,routine.id); refreshProfile(); }} aria-label={`حذف اختصار ${routine.name}`}><Trash2 /></button></article>)}</div></details> : null}
                {workflows.length ? <details className="smart-guide-fold"><summary><BrainCircuit /><span><small>تعلّمت نمطك</small><strong>مسارات معتادة</strong></span><b dir="ltr">{workflows.length}</b></summary><div className="smart-guide-workflows">{workflows.map((workflow,index) => <article key={index}><div><strong>{workflow.sequence.map(id => featureById(id)?.title || id.replace(/^page\./,"")).slice(-4).map((name, stepIndex, names) => <React.Fragment key={`${name}-${stepIndex}`}><span>{name}</span>{stepIndex < names.length - 1 ? <ChevronLeft className="guide-flow-arrow" aria-hidden="true" /> : null}</React.Fragment>)}</strong><small>تكرر {countOf(workflow.count, oblique(AR.visit))}؛ لذلك لا أعتبره تعثرًا.</small></div><div><button type="button" onClick={() => replayWorkflow(workflow.sequence)}><Play />فتح المسار</button><button type="button" onClick={() => setRoutineDraft({sequence:workflow.sequence,name:"مساري المعتاد"})}><Plus />حفظ كاختصار</button></div></article>)}</div></details> : null}
            ...
            {browseMode === "new" ? (
              <>
                <section className="smart-guide-section-head smart-guide-new-head"><div><small>ما الجديد لك</small><strong>{unreadSummary.total ? `${countOf(unreadSummary.total, AR.element)} ${nounFor(unreadSummary.total, AR.unreadAdj)}` : "أنت مطّلع على التحديثات المتاحة"}</strong></div>{unreadSummary.total ? <button type="button" className="smart-guide-mark-all" onClick={markAllNewSeen} aria-label="اعتبر الكل مقروءًا"><Check aria-hidden="true" /><span>إخفاء العداد</span></button> : null}</section>
                <div className="smart-guide-new-summary" aria-label="نطاق عداد الجديد">
                  <article className="is-product"><Sparkles /><span><strong>{unreadSummary.product.length.toLocaleString("ar-KW-u-nu-latn")}</strong><small>{nounFor(unreadSummary.product.length, AR.update)} للمنتج ضمن صلاحياتك</small></span></article>
                  <article className="is-runtime"><MousePointer2 /><span><strong>{unreadSummary.runtime.length.toLocaleString("ar-KW-u-nu-latn")}</strong><small>{nounFor(unreadSummary.runtime.length, AR.element)} {nounFor(unreadSummary.runtime.length, AR.newAdj)} في هذه الشاشة فقط</small></span></article>
                </div>
            ...
            {root && collectiveFriction.length && browseMode === "all" ? <details className="smart-guide-fold"><summary><Gauge /><span><small>تحسين المنتج</small><strong>نقاط التعثر الجماعية</strong></span><b dir="ltr">{collectiveFriction.length}</b></summary><div className="smart-guide-friction">{collectiveFriction.map((item,index) => <article key={`${item.name}-${index}`}><Gauge /><span><strong>{item.name}</strong><small>{countOf(item.count, AR.anonymousSignal)}</small></span><i style={{"--guide-friction":`${Math.min(100,item.count*8)}%`} as React.CSSProperties}/></article>)}</div></details> : null}
