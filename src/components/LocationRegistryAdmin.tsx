...
const numericIdSet=(values:unknown):Set<number>=>{
  ...
  for(const raw of values){const id=Number(raw);if(Number.isFinite(id)&&id>0)ids.add(id);}
  return ids;
};
...
export default function LocationRegistryAdmin({header,demoReadOnly=false}:{header?:React.ReactNode;demoReadOnly?:boolean}){
  ...
  const buildingCollegeIds=(building:MasterBuilding):number[]=>{
    ...
    if(prefix){
      ...
      for(const college of data.colleges){
        ...
        if(Number.isFinite(id)&&id>0)officialIds.add(id);
      }
      if(officialIds.size)return Array.from(officialIds);
    }
  ...
  const buildings=useMemo(()=>data.buildings.filter(building=>{
    ...
    const bCollegeIdSet=new Set<number>(bCollegeIds),bSectionIdSet=new Set<number>(bSectionIds);
    ...
    if(sectionFilter&&!bSectionIdSet.has(sectionFilter))return false;
    return true;
  ...
  return <div className="location-admin-page">
    ...
    {workspaceTab==="registry"?<><div className="location-filter-summary"><span>النتيجة: <b>{buildings.length}</b> من {data.buildings.length} مبنى</span>{selectedCollegeGroup?<Badge>{selectedCollegeGroup.label}</Badge>:null}{sectionFilter?<Badge>{sectionName(sectionFilter)}</Badge>:null}</div>
