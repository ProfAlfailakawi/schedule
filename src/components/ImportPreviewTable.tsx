...
import { AlertTriangle, Building2, Check, Clock3, Pencil, Pin, Search, Trash2, UsersRound } from "lucide-react";
...
export default function ImportPreviewTable({
  ...
  const nextSectionForCourse = (index: number, courseId: number) => {
    const used = new Set(rows
      .filter((row, at) => at !== index && Number(row.AdCourseId) === Number(courseId))
      .map(row => Number(String(row.SCode || "").replace(/\D/g, "")))
      .filter(value => Number.isFinite(value) && value >= 501));
    let candidate = 501;
  ...
  const beginEdit = (index: number) => {
    ...
    const current = Number(String(row?.SCode || "").replace(/\D/g, ""));
    if (!current || current < 501) patch(index, { SCode: nextSectionForCourse(index, Number(row?.AdCourseId) || 0) });
    setCourseQuery("");
    setServerConflicts([]);
    setEditing(index);
  };
  ...
  const localConflicts = useMemo(() => {
    ...
    rows.forEach((other, at) => {
      ...
      if (row.AdInstructorId && Number(row.AdInstructorId) === Number(other.AdInstructorId)) {
        notes.push({ type: "instructor", severity: "high", message: "تعارض أستاذ", detail: `${instructorById.get(Number(row.AdInstructorId))?.AdInstructorName || "الأستاذ"} لديه موعد آخر في الوقت نفسه داخل الملف المستورد.` });
      }
      if (cleanRoom(row.AdRoomCode) && cleanRoom(row.AdRoomHall) && cleanRoom(row.AdRoomCode) === cleanRoom(other.AdRoomCode) && cleanRoom(row.AdRoomHall) === cleanRoom(other.AdRoomHall)) {
        notes.push({ type: "room", severity: "high", message: "تعارض قاعة", detail: `${cleanRoom(row.AdRoomCode)}/${cleanRoom(row.AdRoomHall)} مستخدمة في موعد آخر في الوقت نفسه داخل الملف المستورد.` });
      }
    });
    return notes;
              ...
              <React.Fragment key={`${row.referenceNumber || "row"}-${index}`}>
                <tr className={open ? "is-editing" : ""}>
                    ...
                    {open ? (
                      <div className="import-course-picker">
                        ...
                        <select value={row.AdCourseId || ""} onChange={event => {
                          ...
                          patch(index, { AdCourseId: id, AdCourseName: courseById.get(id)?.CourseName || "", SCode: nextSectionForCourse(index, id) });
                        }}>
                          <option value="">اختر المقرر…</option>
                          {filteredCourses.map(item => <option key={item.AdCourseId} value={item.AdCourseId}>{item.CourseCode} · {item.CourseName}</option>)}
                        </select>
                        <small>{filteredCourses.length.toLocaleString("ar-KW-u-nu-latn")} نتيجة</small>
                      </div>
                    ) : (
                  ...
                  <td className={red(missing.scode(row))}>
                    {open ? <div className="import-section-editor"><input inputMode="numeric" value={String(row.SCode || "")} onChange={event => patch(index, { SCode: event.target.value.replace(/\D/g, "").slice(0, 4) })} /><small>تسلسل الاستيراد لهذا المقرر يبدأ 501 ثم 502 ثم 503…</small></div> : (String(row.SCode || "").trim() || "—")}
                  </td>
                  ...
                  <td className={red(missing.time(row))} dir="ltr">
                    {open ? <span className="import-time-pair"><input type="time" value={row.fendtime || ""} onChange={event => patch(index, { fendtime: event.target.value })} aria-label="وقت النهاية كما يظهر أولاً في ملف PDF" /><i>—</i><input type="time" value={row.fstarttime || ""} onChange={event => patch(index, { fstarttime: event.target.value })} aria-label="وقت البداية كما يظهر ثانياً في ملف PDF" /></span> : (row.fstarttime && row.fendtime ? formatScheduleTimeRange(row.fstarttime, row.fendtime) : "—")}
                  </td>
                  <td className={red(missing.room(row))} dir="ltr">
                    {open ? <div className="import-room-editor"><input list={`import-buildings-${index}`} value={String(row.AdRoomCode || "")} onChange={event => patch(index, { AdRoomCode: event.target.value.toUpperCase().slice(0, 12), AdRoomHall: "" })} placeholder="اختر أو اكتب المبنى" /><datalist id={`import-buildings-${index}`}>{buildings.map(item => <option key={item} value={item} />)}</datalist></div> : (String(row.AdRoomCode || "").trim() || "—")}
                  </td>
                  <td className={red(missing.room(row))} dir="ltr">
                    {open ? <div className="import-room-editor"><input list={`import-halls-${index}`} value={String(row.AdRoomHall || "")} onChange={event => patch(index, { AdRoomHall: event.target.value.toUpperCase().slice(0, 12) })} placeholder="اختر أو اكتب القاعة" /><datalist id={`import-halls-${index}`}>{halls.map(item => <option key={item} value={item} />)}</datalist>{halls.length ? <small>{halls.length.toLocaleString("ar-KW-u-nu-latn")} قاعة محفوظة في هذا المبنى</small> : null}</div> : (String(row.AdRoomHall || "").trim() || "—")}
                  </td>
                ...
                {open && (newRoom || checkingConflicts || conflictNotes.length) ? <tr className="import-row-review"><td colSpan={9}>
