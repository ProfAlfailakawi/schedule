      ...
      case "scheduleChanges":
        ...
        return opensChangesScreen(sessionRole) ? (
          <ScheduleChanges
            ...
            scope={scopes.length === 1 && scopes[0]?.AdSectionId && !["departmentHead", "dean", "viceDean"].includes(sessionRole.id)
        ...
        <nav className="side-nav" aria-label="القائمة الرئيسية">
          ...
          <div className="nav-section nav-section-core" data-rail="core" aria-label="التنقل الأساسي">
            ...
            {/* ── ورشةُ الجدول لمن يبني ─────────────────────────────────────
                مركزُ الذكاء وورشةُ الجدول أدواتُ بناءٍ لمن يبني — اللجنةُ
                والمدير. ورئيسُ القسم لا يبني: يقرأ ويعلّق ويوقّع، وكلُّ ذلك في
                شاشة «تغييرات الجدول»، فلا يُعطى ورشةَ تعديلٍ لا يملك فيها فعلاً.
                وحجبُها عنه يكفيه غيابُ إذنها (allowed.schedule)، فلا يراها. */}
            {allowed.schedule && !sessionRole.viewerOnly ? (
              <NavButton
                activeView={activeView}
                onGo={go}
