...
| N27 | FIXED — AdminUsers shows a warning under «حالة الحساب» when «مدير» is ticked on a read-only role (admin bypasses scope, final-only and redaction) | dean-notify-audit §N27 |
| N28 | FIXED (via N8/N9) — /api/search reads readScopedTermRows (expandScopeSections), /api/search/natural resolves college-wide for a single-college reader; verified against the demo sandbox vice-dean scope | dean-notify-audit §N28 |
