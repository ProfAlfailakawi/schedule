...
app.post("/api/public/survey/:token/identity-status", async (req:Request,res:Response)=>{
  ...
  const civil=asciiDigits(req.body?.civil).replace(/\D/g,""),name=normalizeStudentIdentityName(req.body?.name);
  if(!validateCivilId(civil).isValid||name.length<3){res.json({verified:false});return;}
  const prior=await reusableStudentIdentity(resolved.link,civil);
  if(!prior){res.json({verified:false});return;}
  if(normalizeStudentIdentityName(prior.name)!==name){res.json({verified:true,identityMatched:false});return;}
  ...
  if(!section){res.json({verified:false});return;}
  res.setHeader("Cache-Control","no-store");
  res.json({verified:true,identityMatched:true,sectionId:Number(prior.sectionId),sectionName:String((section as any).AdSectionName||"")});
});
...
(function(){var TOKEN=${JSON.stringify(token)},data=null,step=1,student={name:"",civil:"",sectionId:0},kind="",picked=[],otherCourse=0,proofToken="",proofEligible=false,identityLocked=false,identityMemoryKey="schedule-student-identity-"+TOKEN,identityProbeSeq=0;var host=document.getElementById("host");
...
function loadIdentityMemory(){try{var saved=JSON.parse(localStorage.getItem(identityMemoryKey)||"null");if(!saved)return false;var civil=digits(saved.civil),sectionId=Number(saved.sectionId)||0,name=String(saved.name||"").trim();var allowed=(data.sections||[]).some(function(row){return Number(row.id)===sectionId});if(!civilValid(civil)||name.length<3||!sectionId||!allowed)return false;student={name:name,civil:civil,sectionId:sectionId};identityLocked=true;return true}catch(e){return false}}
...
function clearIdentityMemory(){try{localStorage.removeItem(identityMemoryKey)}catch(e){}identityLocked=false;student={name:"",civil:"",sectionId:Number(data&&data.sectionId)||0};identity()}
function probePriorIdentity(finalCheck){if(identityLocked)return Promise.resolve({locked:true});var name=String((document.getElementById("name")||{}).value||"").trim(),civil=digits((document.getElementById("civil")||{}).value||"");if(name.length<3||!civilValid(civil))return Promise.resolve({locked:false});var seq=++identityProbeSeq;return fetch('/api/public/survey/'+encodeURIComponent(TOKEN)+'/identity-status',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:name,civil:civil})}).then(function(r){return r.json().then(function(d){return{ok:r.ok,d:d}})}).then(function(x){if(seq!==identityProbeSeq)return{locked:false,stale:true};if(!x.ok){if(finalCheck)fail(x.d.error||"تعذر التحقق من بياناتك السابقة");return{locked:false,error:true}}if(!x.d.verified)return{locked:false};if(!x.d.identityMatched){if(finalCheck)fail("بياناتك الأساسية متحققة سابقًا؛ استخدم الاسم نفسه وسيعيد النظام قسمك المعتمد تلقائيًا.");return{locked:false,mismatch:true}}student={name:name,civil:civil,sectionId:Number(x.d.sectionId)||0};identityLocked=true;rememberIdentity();identity();return{locked:true}}).catch(function(){if(finalCheck)fail("تعذر التحقق من بياناتك السابقة. تحقق من الاتصال ثم أعد المحاولة.");return{locked:false,error:true}})}

function identity(){step=1;paintProgress();var locked=identityLocked;host.innerHTML='<div class="step-head"><b>1</b><div><strong>بيانات الطالب</strong><span>الاسم · الرقم المدني · القسم العلمي</span></div></div><div class="fields"><div class="field"><label>الاسم الكامل</label><input id="name" autocomplete="name" value="'+esc(student.name)+'"'+(locked?' readonly aria-readonly="true"':'')+'></div><div class="field"><label>الرقم المدني</label><input id="civil" dir="ltr" inputmode="numeric" maxlength="12" value="'+esc(student.civil)+'"'+(locked?' readonly aria-readonly="true"':'')+'><small id="civilHint" class="hint"></small></div><div class="field full"><label>القسم العلمي</label><select id="section"'+(locked?' disabled aria-disabled="true"':'')+'><option value="">اختر القسم</option>'+(data.sections||[]).map(function(s){return'<option value="'+s.id+'"'+(Number(s.id)===Number(student.sectionId)?' selected':'')+'>'+esc(s.name)+'</option>'}).join("")+'</select></div>'+(locked?'<div class="identity-verified"><span><b>تم التحقق من بياناتك سابقًا.</b> الاسم والرقم المدني والقسم العلمي محفوظة لهذا الطلب ولا تحتاج إدخالها من جديد.</span><button type="button" class="identity-reset" id="identityReset">استخدام طالب آخر</button></div>':'')+'</div><button class="action" id="next">التالي · نوع الطلب</button><p class="privacy">تظهر هويتك للمسؤول المخوّل فقط، وتُحفظ مشفّرة داخل النظام لخدمة الطلب ومراجعته.</p><div id="err"></div>';var civilBox=document.getElementById("civil"),nameBox=document.getElementById("name"),hint=document.getElementById("civilHint");function paintCivil(){var value=digits(civilBox.value);if(!value.length){hint.textContent="";hint.className="hint";return}if(value.length<12){hint.textContent="باقي "+(12-value.length)+" رقم";hint.className="hint";return}if(civilValid(value)){hint.textContent=locked?"هوية متحققة":"رقم مدني صحيح";hint.className="hint ok"}else{hint.textContent="هذا الرقم المدني غير صحيح — راجع أرقام بطاقتك";hint.className="hint bad"}}if(!locked){var probeTimer=0,cue=function(){window.clearTimeout(probeTimer);probeTimer=window.setTimeout(function(){probePriorIdentity(false)},260)};civilBox.oninput=function(){this.value=digits(this.value);paintCivil();cue()};nameBox.oninput=cue}paintCivil();var reset=document.getElementById("identityReset");if(reset)reset.onclick=clearIdentityMemory;document.getElementById("next").onclick=function(){if(!locked){student.name=document.getElementById("name").value.trim();student.civil=digits(civilBox.value);student.sectionId=Number(document.getElementById("section").value)||0}if(student.name.length<3)return fail("اكتب اسمك كاملاً");if(student.civil.length!==12)return fail("أدخل الرقم المدني من 12 رقماً");if(!civilValid(student.civil))return fail("هذا الرقم المدني غير صحيح. راجع الأرقام كما هي في بطاقتك المدنية.");if(!student.sectionId)return fail("اختر قسمك العلمي");var proceed=function(){kind="";picked=[];otherCourse=0;proofToken="";proofEligible=false;chooseType()};if(locked){proceed();return}probePriorIdentity(true).then(function(result){if(result&&result.locked){kind="";picked=[];otherCourse=0;proofToken="";proofEligible=false;chooseType();return}if(result&&result.mismatch)return;if(result&&result.error)return;proceed()})}}
...
function showProofUpload(message){var upload=document.getElementById("proofUpload"),title=document.getElementById("proofTitle"),lead=document.getElementById("proofLead"),status=document.getElementById("proofStatus");if(upload)upload.hidden=false;if(title)title.textContent="ارفع صحيفة التخرج";if(lead)lead.textContent="PDF أو صورة واضحة للصفحة الرسمية «الخطة الدراسية / صحيفة التخرج». يجب أن يظهر الرقم المدني والبرنامج والوحدات المجتازة بوضوح. الاسم يساعد في العرض ولا يشترط تطابقه حرفياً. أي مستند آخر لن يُقبل.";if(status){status.className="proof-status";status.textContent=message||"لم يتم التحقق بعد."}proofEligible=false;proofToken="";refreshGraduateSubmit()}
function acceptVerifiedProof(d,reused){var upload=document.getElementById("proofUpload"),title=document.getElementById("proofTitle"),lead=document.getElementById("proofLead"),status=document.getElementById("proofStatus"),options=document.getElementById("graduateOptions");proofEligible=!!d.eligible;proofToken=d.proofToken||"";if(upload)upload.hidden=true;if(title)title.textContent=reused?"تم التحقق مسبقًا":"تم التحقق من صحيفة التخرج";if(lead)lead.textContent=reused?"لا حاجة لرفع الصحيفة مرة أخرى لهذا الفصل والقسم.":"تم اعتماد بيانات الصحيفة لهذه الجلسة.";if(status){status.className="proof-status "+(reused?"reused":"ok");status.textContent=d.message||"تم التحقق، ويمكنك متابعة الطلب."}if(options){options.hidden=false;wireGraduateDetails()}refreshGraduateSubmit()}
