...
ok('157 one-page photographed PDF keeps the frozen one-page OCR identity path', 'const multiPageCourseKeys=images.length>1?optionKeys:[]' in doc_ocr and 'authorityCourseKeys.length&&(codeSpan||codeIndex>=0)' in doc_ocr)
...
ok('170 Authority diff matches imported rows by exact reference before mutable source order', 'The CRN/reference number printed by the Authority is the strongest row' in server and 'if(reference){' in server and 'const byReference=uniqueIndex' in server and 'if(byReference>=0)return byReference' in server)
ok('171 Authority diff never pairs a baseline CRN with a conflicting current CRN through sourceOrder', 'return !currentReference||currentReference===reference' in server and 'A source row with a real CRN that is no longer present is a deletion' in server and 'return -1;' in server)
ok('172 Authority legacy course-section fallback is restricted to rows without a reference number', '!refKey(row?.referenceNumber)&&sectionKey(row)===signature' in server)
ok('173 Authority building diff compares the complete canonical building rather than only the 3-digit site prefix', 'return cleanBuildingCode(valStr)||valStr' in server and 'baseDigits = valStr.replace' not in server)
